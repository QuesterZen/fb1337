...
from fb1337.array import Array, FlatList, MatrixArray, Coordinate, StructuredArray
from fb1337.dictionary import Dictionary
from fb1337.iterators import ListIterator, LoopIterator, Iterator, Slice
...
def truthy_object(obj):
	...
	if isinstance(obj, ListIterator): return len(obj.all_values()) != 0
...
def is_num_collection(obj):
	...
	if isinstance(obj, Iterator):
		if isinstance(obj, Slice): return False
		if isinstance(obj, Array): return all([is_number(x) for x in obj.unique_values()])
		if isinstance(obj, LoopIterator): return True
...
def parse_program_parameter(parameter):
	...
	if is_list_or_tuple(parameter) and \
		...
		# A list of lists
		return ListIterator([ListIterator(l) for l in parameter])
	...
	if is_list_or_tuple(parameter) and \
		...
		new_object = ListIterator([int(x) for x in parameter])
		return new_object

	if is_list_or_tuple(parameter):
		# Any other list is assumed to be a list of objects
		new_object = ListIterator([parse_program_parameter(x) for x in parameter])
		return new_object
...
def return_value(stack_dump):
	def return_value_rec(final, remove_null=True):
		if final == '' or final is None:
			...
			if isinstance(final, MatrixArray):
				if len(final.all_values()) == 1:
					return round(final.all_values()[0], 2)
				else:
					return final.structured_values().round(2).tolist()
			elif not final.can_reshape():
				if len(final.all_values()) == 1:
					return final.all_values()[0]
				elif isinstance(final, ListIterator) and remove_null:
					return [return_value_rec(x, False) for x in final.flat_list() if x != '']
				else:
					return [return_value_rec(x, False) for x in final.flat_list()]
			else:
				return final.structured_values()
		else:
			return final
...
def parameter_match(parameters, type_signature):
	...
	for t, v in zip(type_signature, parameters):
		...
		elif t == 'List' or t == 'ListIterator':
			match_list.append(isinstance(v, ListIterator))
		elif t == 'Matrix':
			match_list.append(isinstance(v, Matrix) or isinstance(v, MatrixArray))
		elif t == 'Array':
...
def apply_type_transformations(parameters, type_signature, fn):
	...
	for t, v in zip(type_signature, parameters):
		...
		elif (t == 'List' or t == 'ListIterator') and isinstance(v, Matrix):
			new_parameters.append(ListIterator(v.values))
...
def convert_collection(obj, new_type):
	"""Takes a python list, int, float, ListIterator, Dictionary or Matrix and converts it to another type"""
	if (isinstance(obj, FlatList) or isinstance(obj, StructuredArray) or isinstance(obj,
		...
		return ListIterator(values).reshape(obj.get_shape())
	...
	if type(obj) is list or type(obj) is tuple:
		values = list(obj)
	elif isinstance(obj, ListIterator):
		values = obj.values
		...
		values = [obj]
	if new_type == 'List':
		return ListIterator([round(x) if type(x) is float or type(x) is Matrix.data_type else x for x in values])
	elif new_type == 'CList':
		return ListIterator([to_char(x) for x in values])
