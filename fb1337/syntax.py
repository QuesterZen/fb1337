...
# syntax.py
# Program syntax tree for fb1337 interpreter


from fb1337.parser import Reader
...
class SyntaxToken:
	...
	def __init__(self, parse_token,
	             token_type=None, value=None,
	             stack_values=None, code_tokens=None, fn_tokens=None, sub_tree=None):

		self.token_type = parse_token[0] if token_type is None else token_type
		self.value = parse_token[1] if value is None else value

		self.comments = parse_token[2]['comments']
		self.code_location = parse_token[2]['code location']
		self.code_text = parse_token[2]['token_code']
		self.parse_token_type = parse_token[0]
...
class SyntaxTree:
	...
	def _build_tree(program):
		...
		parse_tokens = Reader.tokenize(program)
	...
	def _apply_syntax(token, remaining_parse_tokens):
		...
		if blocks:
			...
			while len(remaining_parse_tokens) > 0:
				...
				if end_context: break
		else:
			sub_tree_syntax = None

		return SyntaxToken(token, token_type='fn', value=symbol,
		                   stack_values=stack_values, code_tokens=code_value_syntax, fn_tokens=function_value_syntax,
	...
	def traverse(self, call_fn):
		"""Traverse the tokens of the tree in order"""
		SyntaxTree._traverse(self.tree, call_fn)
